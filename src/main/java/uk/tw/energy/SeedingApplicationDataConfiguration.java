...
import uk.tw.energy.generator.ElectricityReadingGenerator;
...
public class SeedingApplicationDataConfiguration {
    private final String MOST_EVIL_PRICE_PLAN_ID = "price-plan-0";
    private final String RENEWABLES_PRICE_PLAN_ID = "price-plan-1";
    private final String STANDARD_PRICE_PLAN_ID = "price-plan-2";

    @Bean
    public List<PricePlan> pricePlans() {
        final List<PricePlan> pricePlans = new ArrayList<>();
        pricePlans.add(new PricePlan(MOST_EVIL_PRICE_PLAN_ID, "Dr Evil's Dark Energy", BigDecimal.TEN, Collections.emptyList()));
        pricePlans.add(new PricePlan(MOST_EVIL_PRICE_PLAN_ID, "The Green Eco", BigDecimal.valueOf(2), Collections.emptyList()));
        pricePlans.add(new PricePlan(MOST_EVIL_PRICE_PLAN_ID, "Power for Everyone", BigDecimal.ONE, Collections.emptyList()));
        return pricePlans;
    }
    ...
    public Map<String, List<ElectricityReading>> perMeterElectricityReadings() {
        ...
        final ElectricityReadingGenerator electricityReadingGenerator = new ElectricityReadingGenerator();
        smartMeterToPricePlanAccounts()
                .keySet()
                .forEach(smartMeterId -> readings.put(smartMeterId, electricityReadingGenerator.generate(20)));
        return readings;
    }
